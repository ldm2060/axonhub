...
function SortableColumnRow({ id, label }: SortableColumnRowProps) {
  ...
  return (
    <div ref={setNodeRef} style={style} className='bg-background flex items-center gap-3 rounded-lg border p-3'>
      <button
        {...attributes}
        {...listeners}
...
export function DataTableColumnOrderDialog<TData>({ table, open, onOpenChange, getColumnLabel }: DataTableColumnOrderDialogProps<TData>) {
  const { t } = useTranslation();
  const reorderableIds = useMemo(() => getReorderableColumnIds(table), [table]);
  ...
  }, [open, reorderableIds, columnOrder]);
