...
const { reconcileAnimatedQueue } = await import(`data:text/javascript;base64,${Buffer.from(transpiled).toString('base64')}`);
...
test('deduplicates displayed and already queued records', () => {
  const result = reconcileAnimatedQueue([item('2', 2)], [item('3', 3), item('2', 2), item('1', 1)], [item('1', 1)], 10);
...
test('drops queued records that are absent from the latest server page', () => {
  const result = reconcileAnimatedQueue([item('stale', 2), item('kept', 3)], [item('new', 4), item('kept', 3)], [item('old', 1)], 10);
