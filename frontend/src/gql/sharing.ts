...
// Share/Unshare Model Mutations

const SHARE_MODEL_MUTATION = `
  mutation ShareModel($id: ID!, $userIDs: [ID!]!) {
    shareModel(id: $id, userIDs: $userIDs) {
      id
      ownerID
      visibility
      sharedWith
...
const UNSHARE_MODEL_MUTATION = `
  mutation UnshareModel($id: ID!, $userIDs: [ID!]!) {
    unshareModel(id: $id, userIDs: $userIDs) {
      id
      ownerID
      visibility
      sharedWith
...
export function useShareModel() {
  ...
  return useMutation({
    mutationFn: async ({ id, userIDs }: { id: string; userIDs: string[] }) => {
      const data = await graphqlRequest<{
        shareModel: { id: string; ownerID: string; visibility: string; sharedWith: number[] };
      }>(SHARE_MODEL_MUTATION, { id, userIDs });
      return data.shareModel;
    },
    onSuccess: () => {
    ...
    onError: (error) => {
      handleError(error, { context: 'Share Model' });
    },
  });
}
...
export function useUnshareModel() {
  ...
  return useMutation({
    mutationFn: async ({ id, userIDs }: { id: string; userIDs: string[] }) => {
      const data = await graphqlRequest<{
        unshareModel: { id: string; ownerID: string; visibility: string; sharedWith: number[] };
      }>(UNSHARE_MODEL_MUTATION, { id, userIDs });
      return data.unshareModel;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['models'] });
      toast.success(t('share.messages.unshareSuccess'));
    ...
    onError: (error) => {
      handleError(error, { context: 'Unshare Model' });
    },
  });
}
