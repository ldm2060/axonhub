...
test('channel table shows provider quota only for OAuth channel types', () => {
  ...
  const quotaCell = read('features/channels/components/channels-column-cells.tsx');
  ...
  const oauthTypes = quotaCell.match(/const OAUTH_CHANNEL_TYPES\s*=\s*new Set<Channel\['type'\]>\(\[([\s\S]*?)\]\);/)?.[1];
  assert.ok(oauthTypes, 'OAuth channel type Set declaration should exist');
  for (const type of ['codex', 'claudecode', 'antigravity', 'github_copilot', 'xai_subscription', 'commandcode', 'commandcode_anthropic']) {
  ...
  assert.doesNotMatch(quotaCell, /limit\.window\s*=\s*labels\[index\]/, 'xAI windows must not be labeled by array position');
  assert.match(quotaCell, /Math\.abs\(limit\.usageRatio\s*-\s*usageRatio\)/, 'legacy xAI limits should match raw billing usage');
  assert.match(
    quotaCell,
