...
export function mergeOverrideOperations(existing: OverrideOperation[], template: OverrideOperation[]): OverrideOperation[] {
  ...
  for (const op of template) {
    if (isReplacingBodyOverrideOperation(op) && op.path) {
  ...
  for (const existingOp of existing) {
    if (isReplacingBodyOverrideOperation(existingOp) && existingOp.path) {
  ...
  for (const templateOp of template) {
    if (isReplacingBodyOverrideOperation(templateOp) && templateOp.path && emittedTemplatePaths.has(templateOp.path)) {
      continue;
    }
    result.push(templateOp);
  }
...
export function mergeChannelSettingsForUpdate(
  ...
  return {
    ...
    enableSameChannelRetry: pick('enableSameChannelRetry', existing?.enableSameChannelRetry ?? true),
