...
// Run the pure quota-routing-mode mapping helpers and the REAL
...
const { mergeChannelSettingsForUpdate, recallQuotaRoutingMode, quotaRoutingModeSettingsPatch } = await loadTsModule(mergeSource);

// The dialog must consume the helpers from utils/merge (not redefine them),
// and keep them wired into its payload boundaries.
...
test('mapping helpers stay wired into the dialog payload boundaries', () => {
  assert.doesNotMatch(
    source,
    /export function (recallQuotaRoutingMode|quotaRoutingModeSettingsPatch)/,
    'helpers must live in utils/merge.ts, not the dialog'
  );
  assert.match(source, /from '\.\.\/utils\/merge'/);
