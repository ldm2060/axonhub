...
} from '../utils/request-conversation';
...
const ROLE_KEYS: Record<string, string> = {
  system: 'requests.conversation.roles.system',
  user: 'requests.conversation.roles.user',
  assistant: 'requests.conversation.roles.assistant',
  tool: 'requests.conversation.roles.tool',
};
...
function fmtNum(n: number, language: string): string {
  ...
  return (
    <div className={cn('relative', className)}>
      <div className={cn(collapsed && 'max-h-60 overflow-hidden')}>
        <pre className='text-foreground font-mono text-[12.5px] leading-relaxed break-words whitespace-pre-wrap'>{text}</pre>
      </div>
      {collapsed && (
        <div className='from-background pointer-events-none absolute inset-x-0 bottom-7 h-14 bg-gradient-to-t to-transparent' />
      )}
      {isLong && !expandAll && (
        <button
          type='button'
          onClick={() => setExpanded((v) => !v)}
          className='text-muted-foreground hover:text-foreground bg-muted/40 mt-1 flex w-full cursor-pointer items-center justify-center gap-1 rounded-md border px-2 py-1 text-xs transition-colors'
        >
          {expanded
            ? t('requests.conversation.collapseLong')
            : t('requests.conversation.expandLong', {
                count: text.length,
                formattedCount: fmtNum(text.length, i18n.language),
              })}
        </button>
      )}
    </div>
        ...
        if (p && typeof p === 'object') {
          ...
          if (p.type === 'image_url' || p.type === 'image' || p.image_url) {
            ...
            return (
              <div
                key={i}
                className='border-border bg-muted/30 text-muted-foreground inline-flex items-center gap-1 rounded-md border px-2 py-1 font-mono text-[11px]'
              >
                {t('requests.conversation.content.image')} {url ? url.slice(0, 80) : ''}
              </div>
            );
          }
          if (p.type === 'input_audio' || p.type === 'audio') {
            return (
              <div
                ...
                className='border-border bg-muted/30 text-muted-foreground inline-flex items-center gap-1 rounded-md border px-2 py-1 font-mono text-[11px]'
              >
                {t('requests.conversation.content.audio')}
              </div>
            );
          }
          return (
            <pre
              key={i}
              className='bg-muted/30 border-border text-muted-foreground overflow-x-auto rounded-md border p-2 font-mono text-[11.5px]'
            >
              {prettyJsonBlock(p)}
            </pre>
          );
...
function ToolCallCard({ call, resultIndex, showArgs, jumpTo }: ToolCallCardProps) {
  ...
  return (
    <div className='bg-muted/30 rounded-md border border-l-4 border-purple-500/40 p-2.5 pl-3'>
      <div className='flex flex-wrap items-center gap-2'>
        ...
        {resultIndex !== undefined && (
          ...
          >
            {t('requests.conversation.toolCall.resultLink', { index: resultIndex })}
          </button>
        )}
      </div>
      {showArgs && call.arguments && (
        <>
          ...
          >
            {argsOpen ? t('requests.conversation.toolCall.hideArgs') : t('requests.conversation.toolCall.showArgs')}
          </button>
          {argsOpen && (
            <pre className='bg-muted/40 border-border text-muted-foreground mt-1 overflow-x-auto rounded-md border p-2 font-mono text-[11.5px]'>
              {call.arguments}
            </pre>
          )}
        </>
...
function ToolResultCard({ callIndex, content, jumpTo }: ToolResultCardProps) {
  ...
  return (
    <div className='bg-muted/30 rounded-md border border-l-4 border-orange-500/40 p-2.5 pl-3'>
      <div className='mb-1 flex flex-wrap items-center gap-2'>
        <span className='text-[11px] font-semibold text-orange-600 dark:text-orange-400'>
          {t('requests.conversation.toolResult.label')}
        </span>
        {callIndex !== undefined && (
          <button
            type='button'
            ...
            title={t('requests.conversation.toolResult.jumpToCall')}
          >
            {callIndex} ↑
          </button>
        )}
...
function MessageCard({
  ...
  if (message.content && message.content.length) {
    headMeta.push(
      t('requests.conversation.message.charCount', {
        count: message.content.length,
        formattedCount: fmtNum(message.content.length, i18n.language),
      })
    );
  }
  if (message.reasoning && message.reasoning.length) {
    headMeta.push(
      t('requests.conversation.message.reasoningCount', {
        count: message.reasoning.length,
        formattedCount: fmtNum(message.reasoning.length, i18n.language),
      })
    );
  }
  if (message.toolCalls && message.toolCalls.length) {
    headMeta.push(t('requests.conversation.message.toolCallCount', { count: message.toolCalls.length }));
  }
  ...
  if (message.reasoning && showReasoning) {
    body = (
      <div className='space-y-2.5'>
        <div className='rounded-md border border-dashed border-amber-500/40 bg-amber-500/5 p-2.5'>
          <div className='mb-1 text-[11px] font-semibold tracking-wider text-amber-600 dark:text-amber-400'>
            ◆ {t('requests.conversation.reasoning.label')}
          </div>
          <details className='group'>
            <summary className='text-muted-foreground cursor-pointer text-[11.5px] underline decoration-dotted underline-offset-2'>
              {t('requests.conversation.reasoning.expand')}
            </summary>
  ...
  if (!body) body = <span className='text-muted-foreground italic'>{t('requests.conversation.message.empty')}</span>;
  ...
  return (
    <div id={`conv-msg-${message.index}`} className={cn('border-border bg-muted/20 overflow-hidden rounded-lg border', borderClass)}>
      <div className='border-border flex flex-wrap items-center gap-2 border-b px-3 py-2'>
        <span className={cn('inline-flex items-center rounded-full px-2 py-0.5 font-mono text-[11px]', pillClass)}>
          {t(ROLE_KEYS[role] ?? 'requests.conversation.roles.other', { role })}
        </span>
        ...
        <div className='ml-auto flex flex-wrap items-center gap-1.5'>
          {headMeta.map((meta, i) => (
            <span
              key={i}
              className='border-border bg-muted/40 text-muted-foreground rounded-full border px-2 py-0.5 font-mono text-[10.5px]'
            >
              {meta}
            </span>
          ))}
          <Button
            ...
            size='icon-sm'
            className='h-6 w-6'
            onClick={() => onToggleRaw(message.index)}
            title={t('requests.conversation.viewRaw')}
          >
      ...
      {rawOpen && (
        <div className='border-border bg-muted/40 border-b px-3 py-2.5'>
          <div className='text-muted-foreground mb-1 font-mono text-[10.5px]'>
            {t('requests.conversation.message.rawTitle', { index: message.index })}
          </div>
          <pre className='text-muted-foreground max-h-72 overflow-auto font-mono text-[11.5px] leading-relaxed break-words whitespace-pre-wrap'>
...
function ToolCard({ tool }: ToolCardProps) {
  ...
  return (
    <div className='border-border bg-muted/20 overflow-hidden rounded-lg border'>
      ...
      >
        <Wrench className='h-4 w-4 shrink-0 text-cyan-600 dark:text-cyan-400' />
        ...
        <span className='text-muted-foreground shrink-0 font-mono text-[11px]'>
          {t('requests.conversation.tool.propertyCount', { count: propCount })}
          {required ? ` · ${t('requests.conversation.tool.requiredList', { fields: required })}` : ''}
        </span>
      ...
      {open && (
        <div className='border-border border-t p-3.5'>
          ...
          <div className='text-muted-foreground mb-1 font-mono text-[10.5px]'>
            {t('requests.conversation.tool.parameters')} · {t('requests.conversation.tool.required')}:{' '}
            <span className='text-orange-600 dark:text-orange-400'>{required || '—'}</span>
          </div>
          <pre className='bg-muted/40 border-border text-muted-foreground overflow-x-auto rounded-md border p-2.5 font-mono text-[11.5px]'>
...
function Stat({ label, value, jumpId, onJump }: { label: string; value: React.ReactNode; jumpId?: string; onJump?: (id: string) => void }) {
  return (
    <div
      className={cn('border-border bg-muted/20 rounded-lg border px-3 py-2', jumpId && 'hover:border-primary cursor-pointer')}
    ...
    <div ref={rootRef} className={cn('space-y-4', className)}>
      ...
      <div className='border-border bg-muted/20 sticky top-0 z-10 rounded-lg border p-3 backdrop-blur'>
        <div className='flex flex-wrap items-center gap-2.5'>
          ...
          {data.model && (
            <span className='rounded-full border border-blue-500/40 bg-blue-500/10 px-2.5 py-0.5 font-mono text-[11.5px] text-blue-600 dark:text-blue-400'>
              {data.model}
            </span>
          )}
          <div className='ml-auto flex flex-wrap items-center gap-2'>
            ...
            <label className='text-muted-foreground flex cursor-pointer items-center gap-1.5 text-xs whitespace-nowrap'>
              <input
                type='checkbox'
                className='accent-blue-500'
                checked={showToolArgs}
                onChange={(e) => setShowToolArgs(e.target.checked)}
              />
            ...
            <label className='text-muted-foreground flex cursor-pointer items-center gap-1.5 text-xs whitespace-nowrap'>
              <input
                type='checkbox'
                className='accent-blue-500'
                checked={showToolResult}
                onChange={(e) => setShowToolResult(e.target.checked)}
              />
            ...
            <label className='text-muted-foreground flex cursor-pointer items-center gap-1.5 text-xs whitespace-nowrap'>
              ...
              {t('requests.conversation.toggleSystem')}
            </label>
      ...
      <div className='grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-5'>
        <Stat label={t('requests.conversation.statModel')} value={data.model ? <small className='text-xs'>{data.model}</small> : '—'} />
        <Stat label={t('requests.conversation.statMessages')} value={fmtNum(data.messages.length, i18n.language)} />
        <Stat
          label={t('requests.conversation.statTools')}
          value={fmtNum(data.tools.length, i18n.language)}
          jumpId='conv-tools'
          onJump={jumpTo}
        />
          ...
          value={
            <span className='flex flex-wrap gap-1'>
              {Object.entries(roles).map(([r, c]) => (
                <span
                  key={r}
                  className={cn('inline-flex items-center rounded-full px-1.5 py-0.5 font-mono text-[10.5px]', ROLE_PILL_CLASSES[r] || '')}
                >
                  {r} {c}
                </span>
              ))}
            ...
            {sidebarGroups.map((g) => (
              <div key={g.role} className='mb-1.5'>
                <div className='text-muted-foreground px-1.5 py-0.5 font-mono text-[10.5px] tracking-wider uppercase'>
                  {ROLE_ICON[g.role]} {t(ROLE_KEYS[g.role] ?? 'requests.conversation.roles.other', { role: g.role })} ({g.items.length})
                </div>
                {g.items.map((item) => (
                  <button
                    key={item.index}
        ...
        <div className='min-w-0 space-y-5'>
          <div>
            <div className='text-muted-foreground mb-2.5 text-[13px] font-semibold'>
              {t('requests.conversation.messagesTitle')}{' '}
              <span className='text-muted-foreground/70'>({fmtNum(visibleMessages.length, i18n.language)})</span>
            </div>
          ...
          <div id='conv-tools' className='scroll-mt-24'>
            <div className='text-muted-foreground mb-2.5 text-[13px] font-semibold'>
              {t('requests.conversation.toolsTitle')}{' '}
              <span className='text-muted-foreground/70'>({fmtNum(data.tools.length, i18n.language)})</span>
            </div>
            {data.tools.length === 0 ? (
