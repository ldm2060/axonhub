...
const AddChannelDialog = lazy(() =>
  import('./components/add-channel-dialog').then((m) => ({ default: m.AddChannelDialog })),
);
const EditChannelDialog = lazy(() =>
  import('./components/edit-channel-dialog').then((m) => ({ default: m.EditChannelDialog })),
);
const DeleteChannelDialog = lazy(() =>
  import('./components/delete-channel-dialog').then((m) => ({ default: m.DeleteChannelDialog })),
);
    ...
    return (
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
        {Array.from({ length: 4 }).map((_, i) => (
          <div
            key={i}
            className="h-48 animate-pulse rounded-lg border bg-muted p-4"
          />
        ))}
      </div>
    );
  ...
  if (channels.length === 0) {
    return (
      <div className="flex flex-1 flex-col items-center justify-center gap-4 py-16">
        <p className="text-sm text-muted-foreground">{t('usageMonitor.noData')}</p>
        <Button onClick={() => setOpen('add')}>
          <Plus className="mr-2 size-4" />
          {t('usageMonitor.addChannel')}
        </Button>
      ...
      <Header fixed>
        <div className="flex w-full flex-1 flex-col gap-2 md:flex-row md:items-center md:justify-between md:gap-0">
          <div className="min-w-0">
            ...
            <p className="text-sm text-muted-foreground">{t('usageMonitor.description')}</p>
          </div>
