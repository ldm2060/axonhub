...
export function AddChannelDialog() {
  ...
  async function handleSubmit() {
    try {
      ...
      const legacyFields: FieldConfig[] = source === 'template' ? (selectedTemplate?.fields ?? []) : [];
  ...
  const canSubmit =
    name.trim() && apiUrl.trim() && !headersError && (source === 'template' ? selectedTemplate !== null && apiKey.trim() !== '' : true);
  return (
    <Dialog
      open={isOpen}
      onOpenChange={(v) => {
          ...
          <Tabs value={source} onValueChange={(v) => resetForSource(v as SourceType)} className='gap-4'>
            <TabsList className='w-full'>
              <TabsTrigger value='builtin' className='flex-1'>
                {t('usageMonitor.tabs.builtin')}
              </TabsTrigger>
              <TabsTrigger value='custom' className='flex-1'>
                {t('usageMonitor.tabs.custom')}
              </TabsTrigger>
              <TabsTrigger value='template' className='flex-1'>
                {t('usageMonitor.tabs.template')}
              </TabsTrigger>
            </TabsList>

            {/* ========== Builtin Tab ========== */}
            <TabsContent value='builtin' className='mt-0 space-y-5 pb-4'>
              {/* Channel Selector */}
            ...
            {/* ========== Custom Tab ========== */}
            <TabsContent value='custom' className='mt-0 space-y-5 pb-4'>
              {/* API URL */}
              <div className='space-y-1.5'>
                <Label>{t('usageMonitor.apiUrl')}</Label>
                <Input
            ...
            {/* ========== Template Tab ========== */}
            <TabsContent value='template' className='mt-0 space-y-5 pb-4'>
              {/* Template Selector */}
              <div className='space-y-1.5'>
              ...
              {selectedTemplate && (
                <>
                  {selectedTemplate.description && <p className='text-muted-foreground text-xs'>{selectedTemplate.description}</p>}
                  ...
                  {authType === 'device_flow' && selectedTemplate.providerType === 'github_copilot' && (
                    <div className='space-y-1.5'>
                      ...
                      <CopilotDeviceFlow onSuccess={(credentials) => setApiKey(credentials)} existingCredentials={apiKey} />
                    </div>
                  )}
                      ...
                      {claudecodeOAuth.authUrl && (
                        <div className='space-y-2'>
                          ...
                          <Button
                            type='button'
                            variant='secondary'
                            onClick={claudecodeOAuth.exchange}
                            disabled={claudecodeOAuth.isExchanging}
                          >
                            {claudecodeOAuth.isExchanging
                              ? t('channels.dialogs.oauth.buttons.starting')
                              : t('channels.dialogs.oauth.buttons.startOAuth')}
                          </Button>
                        </div>
                      )}
                      {!claudecodeOAuth.authUrl && (
                  ...
                  <div className='space-y-3'>
                    ...
                    <p className='text-muted-foreground text-xs'>{t('usageMonitor.templateVariablesHint')}</p>
                  ...
                  <div className='space-y-3'>
                    ...
                    <p className='text-muted-foreground text-xs'>{t('usageMonitor.templateDisplayFieldsHint')}</p>
                    <DisplayFieldForm displayFields={displayFields} variables={variables} onChange={setDisplayFields} />
                  </div>
