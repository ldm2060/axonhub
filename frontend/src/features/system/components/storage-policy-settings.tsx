...
function groupBodyPreviewItems(items: GcCleanupPreviewItem[]): BodyPreviewGroup[] {
  ...
  for (const item of items) {
    ...
    const key = item.retentionDays > 0 ? `days:${item.retentionDays}` : `day:${new Date(item.cutoffTime).toISOString().slice(0, 10)}`;
...
function upsertCleanupOption(options: CleanupOption[], resourceType: string, patch: Partial<CleanupOption>): CleanupOption[] {
...
export function StoragePolicySettings() {
  ...
  const fetchPreview = React.useCallback(
    async (reqDays: number, usageDays: number, requestBodyDays: number, responseBodyDays: number, chunkDays: number) => {
  ...
  const schedulePreview = (reqDays: number, usageDays: number, requestBodyDays: number, responseBodyDays: number, chunkDays: number) => {
  ...
  const hasChanges =
    ...
    (storagePolicy.storeChunks !== storagePolicyState.storeChunks ||
      ...
      JSON.stringify(ensureCleanupOptions(storagePolicy.cleanupOptions)) !== JSON.stringify(storagePolicyState.cleanupOptions));
  ...
  const renderManualDaysRow = (resourceType: string, value: number, onChange: (value: number) => void) => (
    <div className='flex items-center gap-4'>
      <Label className='w-40 min-w-36 shrink-0'>{resourceTypeLabel(resourceType)}</Label>
          ...
          <AlertDialog onOpenChange={handleDialogOpenChange}>
            ...
            <AlertDialogContent className='max-h-[90vh] overflow-y-auto'>
              ...
              <div className='space-y-5 py-4'>
                ...
                <div className='rounded-lg border p-3'>
                  ...
                  {isPreviewLoading ? (
                    ...
                    <div className='text-muted-foreground text-sm'>{t('system.storage.policy.runCleanupPreviewFailed')}</div>
                      ...
                      {(() => {
                        ...
                        return (
                          <div className='space-y-2'>
                            <div className='text-sm font-medium'>{t('system.storage.policy.runCleanupPreviewScanLabel')}</div>
                            ...
                            <div className='text-muted-foreground text-xs'>{t('system.storage.policy.runCleanupPreviewBodyHint')}</div>
                          </div>
                        );
                      })()}
                    </div>
              ...
              <AlertDialogFooter>
                ...
                <AlertDialogAction onClick={handleManualCleanup} disabled={triggerGcCleanup.isPending}>
                ...
                <div className='flex items-center justify-between'>
                  <div className='space-y-0.5'>
                    ...
                    {cleanupOptionDescription(option.resourceType) ? (
                      <div className='text-muted-foreground text-sm'>{cleanupOptionDescription(option.resourceType)}</div>
                    ) : null}
                  </div>
                  <Switch
