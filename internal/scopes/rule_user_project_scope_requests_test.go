...
func TestProjectOwnerCanReadMemberPersonalKeyRequests(t *testing.T) {
	...
	// A regular member lacks the system read-users scope, so they cannot list
	// other project members. Our UserOwnedQueryRule (UserSelfFilter) still lets
	// them read their own User record, so the query succeeds but is filtered to
	// only themselves (count == 1), rather than returning an error.
	memberSelfCount, err := client.User.Query().Count(memberCtx)
	require.NoError(t, err)
	require.Equal(t, 1, memberSelfCount)
