...
func serveResponsesWebSocket(
	...
	defer conn.Close()
	writer := &responsesWebSocketWriter{conn: conn, mu: sync.Mutex{}}
...
func (d *responsesWebSocketDispatcher) lane(streamID string) (*responsesWebSocketLane, *httpclient.Error) {
	...
	close(ready)
	lane := &responsesWebSocketLane{session: responsesWebSocketSession{warmupID: "", warmupPayload: nil}, tail: ready}
