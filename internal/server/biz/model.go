...
// ShareModel shares a model with the specified users.
func (svc *ModelService) ShareModel(ctx context.Context, modelID, ownerID int, userIDs []int) (*ent.Model, error) {
	client := svc.entFromContext(ctx)
	m, err := client.Model.Get(ctx, modelID)
	if err != nil {
		return nil, fmt.Errorf("model not found: %w", err)
	}
	if m.OwnerID == 0 || m.OwnerID != ownerID {
		return nil, fmt.Errorf("only the owner can share this model")
	}
	...
	for _, uid := range userIDs {
		...
		if !lo.Contains(sharedWith, uid) {
			sharedWith = append(sharedWith, uid)
		}
	}

	m, err = client.Model.UpdateOneID(modelID).
		SetSharedWith(sharedWith).
		SetVisibility(model.VisibilityShared).
		Save(ctx)
	if err != nil {
		return nil, fmt.Errorf("failed to share model: %w", err)
	}
	return m, nil
}

// UnshareModel removes users from the shared_with list.
func (svc *ModelService) UnshareModel(ctx context.Context, modelID, ownerID int, userIDs []int) (*ent.Model, error) {
	client := svc.entFromContext(ctx)
	m, err := client.Model.Get(ctx, modelID)
	if err != nil {
		return nil, fmt.Errorf("model not found: %w", err)
	...
	if m.OwnerID == 0 || m.OwnerID != ownerID {
		return nil, fmt.Errorf("only the owner can unshare this model")
	}

	sharedWith := lo.Filter(m.SharedWith, func(id int, _ int) bool {
		return !lo.Contains(userIDs, id)
	})
	...
	m, err = client.Model.UpdateOneID(modelID).
		...
		Save(ctx)
	if err != nil {
		return nil, fmt.Errorf("failed to unshare model: %w", err)
	}
	return m, nil
}

// ListSharedWithUser returns models shared with the given user.
func (svc *ModelService) ListSharedWithUser(ctx context.Context, userID int) ([]*ent.Model, error) {
	client := svc.entFromContext(ctx)
	models, err := client.Model.Query().
		Where(model.VisibilityEQ(model.VisibilityShared)).
		All(ctx)
	if err != nil {
		return nil, err
	}
	return lo.Filter(models, func(m *ent.Model, _ int) bool {
		return scopes.IsSharedWithUser(m.SharedWith, userID)
	}), nil
}
