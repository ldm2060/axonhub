...
func (svc *ChannelService) evaluateAutoDisableForFailure(ctx context.Context, perf *PerformanceRecord) (matched, acted bool) {
	...
	// Channels saved before the rules engine carry a legacy status-based
	// auto_disable_config. It keeps working: custom+enabled replaces the global
	// layer with its own thresholds (the old semantics), disabled turns
	// auto-disable off for the channel, and inherit falls through to global.
	switch legacy, rules := legacyAutoDisableConfigRules(ch.Channel); legacy {
	case legacyAutoDisableOff:
		return false, false
	case legacyAutoDisableCustom:
		return svc.evaluateRules(ctx, perf, rules, autoDisableScopeChannel)
	}
...
type legacyAutoDisableScope string

const (
	legacyAutoDisableInherit legacyAutoDisableScope = "inherit"
	legacyAutoDisableCustom  legacyAutoDisableScope = "custom"
	legacyAutoDisableOff     legacyAutoDisableScope = "off"
)

// legacyAutoDisableConfigRules translates a channel's legacy status-based
// auto-disable config into rules the engine can evaluate. Rules use the
// permanent action because the legacy config had no expiry concept.
func legacyAutoDisableConfigRules(ch *ent.Channel) (legacyAutoDisableScope, []objects.APIKeyAutoDisableRule) {
	if ch == nil || ch.AutoDisableConfig == nil {
		return legacyAutoDisableInherit, nil
	}

	cfg := ch.AutoDisableConfig

	switch cfg.Mode {
	case objects.AutoDisableModeDisabled:
		return legacyAutoDisableOff, nil
	case objects.AutoDisableModeCustom:
		if !cfg.Enabled || len(cfg.Statuses) == 0 {
			return legacyAutoDisableOff, nil
		}

		rules := make([]objects.APIKeyAutoDisableRule, 0, len(cfg.Statuses))
		...
		return legacyAutoDisableCustom, rules
	default:
		// AutoDisableModeInheritGlobal and unknown modes fall through to global.
		return legacyAutoDisableInherit, nil
	}
}
