...
func buildBaseRequest(chatReq *llm.Request, config *Config) *MessageRequest {
	...
	switch thinkingType {
	case "disabled":
		req.Thinking = &Thinking{Type: "disabled", BudgetTokens: 0, Display: ""}
	case "adaptive":
		req.Thinking = &Thinking{Type: "adaptive", BudgetTokens: 0, Display: ""}
	case "enabled":
...
func convertToLlmResponse(anthropicResp *Message, platformType PlatformType) *llm.Response {
	...
	if len(reasoningItems) == 1 {
		...
		if aggregateReasoning := sb.String(); aggregateReasoning != "" {
