...
func TestOutputConfig_Outbound(t *testing.T) {
	tests := []struct {
		...
		{
			name: "Zhipu platform output_config_effort=max -> Thinking{enabled} budget (legacy)",
			chatReq: &llm.Request{
				Model:     "glm-4.5",
				MaxTokens: lo.ToPtr(int64(4096)),
				Messages: []llm.Message{
			...
			validate: func(t *testing.T, anthropicReq *MessageRequest) {
				t.Helper()
				// Zhipu is a legacy platform without output_config.effort support:
				// the effort metadata is expressed via the thinking budget table.
				require.Nil(t, anthropicReq.OutputConfig)
				require.NotNil(t, anthropicReq.Thinking)
				require.Equal(t, "enabled", anthropicReq.Thinking.Type)
			},
