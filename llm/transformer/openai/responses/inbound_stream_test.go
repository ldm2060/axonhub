...
func TestInboundTransformer_TransformStream_MapsFinishReasonToCompletedStatus(t *testing.T) {
	tests := []struct {
		...
		{name: "cancelled maps to cancelled", finishReason: "cancelled", expectedStatus: "cancelled"},             //nolint:misspell // OpenAI-compatible protocol values
		{name: "canceled (US spelling) maps to cancelled", finishReason: "canceled", expectedStatus: "cancelled"}, //nolint:misspell // OpenAI-compatible protocol values
