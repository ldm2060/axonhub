...
func (s *responsesOutboundStream) transformStreamChunk(event *httpclient.StreamEvent) error {
	...
	case StreamEventTypeResponseCompleted:
		...
		// with a status other than "completed" (incomplete/failed/canceled) - it
		...
		if streamEvent.Response != nil && streamEvent.Response.Status != nil {
			...
			case "failed":
				finishReason = "error"
			case "cancelled", "canceled": //nolint:misspell // OpenAI-compatible protocol value
				finishReason = "cancelled" //nolint:misspell // Chat Completions protocol value
			}
		}
		if finishReason == "" {
